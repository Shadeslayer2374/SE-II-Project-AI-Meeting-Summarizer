...
def summarize_text(text):
    ...
    # Summarize each chunk with dynamic length
    summary = " ".join([
        summarizer(
            chunk,
            max_length=min(100, len(chunk)//2),  # Dynamic max length
            min_length=30,
            do_sample=False
        )[0]["summary_text"]
        for chunk in chunks
...
def create_pdf(transcription, summary, output_path):
    """Create PDF with transcription and summary (Unicode compatible)"""
    pdf = FPDF()
    pdf.add_page()
    ...
    # Add transcription section with Unicode handling
    pdf.set_font("Arial", 'B', size=12)
    pdf.cell(200, 10, txt="Transcription:", ln=1)
    pdf.set_font("Arial", size=10)
    
    try:
        # Try UTF-8 compatible encoding
        pdf.multi_cell(0, 5, txt=transcription.encode('latin-1', 'replace').decode('latin-1'))
    except Exception as e:
        print(f"Encoding error, using safe fallback: {str(e)}")
        pdf.multi_cell(0, 5, txt=transcription.encode('ascii', 'replace').decode('ascii'))
    ...
    try:
        pdf.multi_cell(0, 5, txt=summary.encode('latin-1', 'replace').decode('latin-1'))
    except Exception as e:
        print(f"Encoding error, using safe fallback: {str(e)}")
        pdf.multi_cell(0, 5, txt=summary.encode('ascii', 'replace').decode('ascii'))
...
def process_video_to_pdf(video_path, pdf_output_path):
    ...
    try:
        ...
        if audio_path and os.path.exists(audio_path):
            for _ in range(3):  # Try multiple times on Windows
                try:
