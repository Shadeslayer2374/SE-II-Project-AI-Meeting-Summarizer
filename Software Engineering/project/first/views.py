...
def send_otp(user):
    ...
    sender_password = "AniLeviPrachi2026"  # Consider using environment variables
    ...
    return
    #     return HttpResponse("A password reset OTP has been sent to your email.")
    # except Exception as e:
    #     return HttpResponse(f"Failed to send OTP: {str(e)}", status=500)
...
# File Processing and Summarizing
        ...
        try:
            ...
            try:
                ...
                pdf_status[file_id].update({
                    'status': 'ready',
                    'pdf_path': pdf_record.pdf_file.path,
                    'db_id': pdf_record.pdf_id  
                })
                ...
                return JsonResponse({
                    ...
                    'file_id': file_id,
                    'db_id': pdf_record.pdf_id,
                    'message': f'{file_type.capitalize()} processed successfully'
                })
...
def check_pdf_status(request, file_id):
    status_info = pdf_status.get(file_id, {'status': 'not_found'})
    
    if status_info['status'] == 'processing':
        processing_time = (datetime.now() - status_info['start_time']).total_seconds()
        if processing_time > 6000:
...
def download_pdf(request, file_id):
    status_info = pdf_status.get(file_id)
    
    if not status_info or status_info.get('status') != 'ready':
        return HttpResponse('PDF not ready or not found', status=404)
    
    try:
        pdf_record = Pdf.objects.get(pk=status_info['db_id'])
        response = FileResponse(pdf_record.pdf_file, content_type='application/pdf')
        response['Content-Disposition'] = f'attachment; filename="{pdf_record.pdf_file.name}"'
        ...
        return HttpResponse('PDF not found in database', status=404)
...
def pdf_viewer(request):
    ...
    return render(request, 'pdf_viewer.html', {'file_id': file_id})