...
def profile(request):
    if 'user_id' in request.session:
        ...
        try:
            ...
            # Filter PDFs and ensure they have .pdf extension
            pdfs = Pdf.objects.filter(user=user, pdf_file__endswith='.pdf')            
            # Prepare PDF data with proper URLs
            pdf_list = []
            for pdf in pdfs:
                pdf_list.append({
                    'id': pdf.pdf_id,
                    'name': pdf.name,
                    'url': pdf.pdf_file.url if pdf.pdf_file else '#',
                    'created_at': pdf.created_at.strftime("%b %d, %Y"),
                    'size': f"{pdf.pdf_file.size / 1024:.1f} KB" if pdf.pdf_file else 'N/A'
                })
            ...
            context = {
                ...
                "pdfs": pdf_list,  # Use the processed list instead
            }
...
def process_file(request):
    if request.method == 'POST':
        ...
        try:
            ...
            # Write the file with proper file handling
            ...
            try:
                ...
                # Save PDF with proper file handling
                ...
                # Cleanup in case of processing error
                def safe_remove(filepath):
                    try:
                        if filepath and os.path.exists(filepath):
                            os.remove(filepath)
            ...
            if original_path and os.path.exists(original_path):
                try:
                    ...
                    print(f"Final cleanup failed for {original_path}: {e}")
...
def delete_pdf(request, pdf_id):
    if request.method == 'DELETE':
        try:
            ...
            # Get the user and PDF
            user = Account.objects.get(Email=request.session['email'])
            pdf = Pdf.objects.get(pdf_id=pdf_id, user=user)
            ...
            # Decrement the uploads count and get the updated value
            user.uploads = F('uploads') - 1
            user.save()
            
            # Refresh the user object to get the updated uploads count
            user.refresh_from_db()
            
            return JsonResponse({
                'success': True, 
                'message': 'PDF deleted successfully',
                'new_uploads_count': user.uploads  # Now contains the actual value
            })
            
        except (Account.DoesNotExist, Pdf.DoesNotExist):
            return JsonResponse({'success': False, 'message': 'PDF not found'}, status=404)
        except Exception as e:
